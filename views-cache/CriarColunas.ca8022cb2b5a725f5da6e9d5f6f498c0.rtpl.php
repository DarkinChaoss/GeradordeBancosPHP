...
<div class="btn-group">
  ...
  <div class="dropdown-menu">
    <a class="dropdown-item" href="/insert1">Inserir dados</a>
  </div>
</div>
...
<form class="form-inline" action="/crcol" method="post">
  ...
  <select name="valor" class="custom-select mb-2 mr-sm-2 mb-sm-0" id="inlineFormCustomSelect">
    <option value="int(40)">int</option>
    <option value="double(40)">double</option>
    <option value="float(40)">float</option>
    <option value="char(80)">char</option>
  </select>
  ...
  <select name="nomeT" class="custom-select mb-2 mr-sm-2 mb-sm-0" id="inlineFormCustomSelect">
    {<?php $counter1=-1;  if( isset($dados2) && ( is_array($dados2) || $dados2 instanceof Traversable ) && sizeof($dados2) ) foreach( $dados2 as $key1 => $value1 ){ $counter1++; ?>}<option value="<?php echo htmlspecialchars( $value1["Tables_in_banco"], ENT_COMPAT, 'UTF-8', FALSE ); ?>"><?php echo htmlspecialchars( $value1["Tables_in_banco"], ENT_COMPAT, 'UTF-8', FALSE ); ?> </option><?php } ?>
  </select>
